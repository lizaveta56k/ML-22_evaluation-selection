...
def create_pipeline(
    ...
    if use_random_fores_classifier :
        ...
        selector = SequentialFeatureSelector(KNeighborsClassifier(n_neighbors=n_neighbors), n_features_to_select=n_features_to_select)

    if use_variance_threshold or use_random_fores_classifier or use_sequential_feature_selector:
        pipeline_steps.append(("selector", selector))
    if use_feature_reduction:
        pipeline_steps.append(("feature_reduction", TruncatedSVD(n_components=n_features_to_select, n_iter=n_iter, random_state=random_state)))
    if use_scaler:
